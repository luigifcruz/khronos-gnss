...
void WebSockets::HttpServe(struct netconn *conn, void* task) {
    struct netbuf* inbuf;
    static char* buf;
    static uint16_t buflen;
    static err_t err;

    WebSockets* that = (WebSockets*)task;
    printf("VALUE IS: %d\n", that->db->GetSettings().ws_update_rate);
    ...
    if (err == ERR_OK) {
        ...
        if (buf && strstr(buf,"GET / ") && strstr(buf,"Upgrade: websocket")) {
            ws_server_add_client(conn, buf, buflen, "/", task, WebSockets::WebSocketCallback);
            netbuf_delete(inbuf);
        } else {
...
void WebSockets::ServerHandleQueue(void* taskStartParameters) {
    struct netconn* conn;
    
    while (1) {
        xQueueReceive(client_queue, &conn, portMAX_DELAY);
        if(!conn) continue;
        HttpServe(conn, taskStartParameters);
    }
    vTaskDelete(NULL);
}
